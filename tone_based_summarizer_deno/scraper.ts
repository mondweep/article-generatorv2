...
import { config } from "./config.ts"; // Import config for API key
...
async function scrapeWithScrapingBee(url: string): Promise<ScrapeResult | null> {
    console.log(`Attempting ScrapingBee API for: ${url}`);
    const scrapingBeeUrl = "https://app.scrapingbee.com/api/v1/";
    const apiKey = config.scrapingApiKey;
 ...
 */
export async function scrapeUrlContent(url: string): Promise<ScrapeResult | null> {
    console.log(`Attempting to scrape content from: ${url}`);

    // Try direct fetch first
    let result = await scrapeWithDirectFetch(url);

    // If direct fetch failed (returned null), try ScrapingBee
    if (result === null) {
        console.log(`Direct fetch failed for ${url}, falling back to ScrapingBee.`);
        result = await scrapeWithScrapingBee(url);
    }
